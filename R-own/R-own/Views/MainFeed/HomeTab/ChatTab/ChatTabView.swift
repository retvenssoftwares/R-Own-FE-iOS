...
struct ChatTabView: View {
    ...
    var destinationView: some View {
        if message.isGroupMessage() {
            GroupMessageView(loginData: loginData, communityVM: communityVM, mesiboVM: mesiboData, groupID: String(message.groupProfile?.getGroupId() ?? 0), communityName: message.groupProfile?.getName() ?? "", communityPic: message.groupProfile?.getImageUrl() ?? "", globalVM: globalVM, profileVM: profileVM)
        } else {
...
//                ChatTabStruct(mesiboVM: mesiboData, message: message)
//            })
//        }
//        .onAppear{
////            firstFourLetter = getFirstFourLetters(text.message ?? "")
